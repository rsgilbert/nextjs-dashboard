...
export async function fetchLatestInvoices() {
    ...
    try {
      ...
      ORDER BY invoices.date, customers.name DESC
      LIMIT 5`;
...
export async function fetchTotalPaidInvoices(): Promise<number> {
    const queryResult = await sql<{ count: number }>`SELECT COUNT(*) AS count FROM invoices`;
    const result = queryResult.rows[0].count
    return result;
}
